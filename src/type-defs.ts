...
type Product
    ...
    #
    # The scope is repeated here because a field's hint replaces the return type's, rather than
    # adding to it: without it this would say nothing about audience, and a hint that says nothing
    # is read as private, so the whole product answer would be stored once per caller.
...
type Category
    ...
    #
    # And the scope again, for the same reason: this hint replaces \`Product\`'s, so leaving it out
    # would drop the \`PUBLIC\` written there and store a public catalogue per caller.
...
type Activity @cacheControl(maxAge: 5, scope: PUBLIC, swr: 60) @surrogateKey(static: "activity") {
    ...
    #
    # \`scope\` is written here as well, and it has to be. A field's own hint replaces the return
    # type's hint, it is not merged with it, so this annotation hides \`ActivityEntry\` entirely. A
    # hint that names no scope is read as private, which would quietly give this public feed one
    # stored entry per caller.
...
type Query {
    ...
    #
    # No \`@cacheControl\` here on purpose. A field's hint REPLACES its return type's rather than
    # adding to it, so an annotation repeating \`type Fault\`'s numbers would be a second place to
    # change and one place to forget. Left bare, the type's hint applies, which is the same shape
    # \`product(id:)\` above relies on.
    #
